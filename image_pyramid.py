...
def pyramid_builder(
    ...
    while image.shape[0] > 1 and image.shape[1] > 1:
        ...
        now_level = image - compose(upsample, interpolaion_filter)(next_level).reshape(
            image.shape
        )
        yield now_level
        image = next_level
...
def merge_image(images: List[np.ndarray]) -> np.ndarray:
    ...
    ret_image = np.ones((size, size * 2, channels)) * 255
    start_x = 0
    for image in images:
        image_size = image.shape[0]
        ret_image[0:image_size, start_x : start_x + image_size, :] = image
        start_x += image_size
...
if __name__ == "__main__":
    ...
    interpolation_filters = {
        "nearest": lambda x: cv2.resize(
            x, (x.shape[0] * 2, x.shape[1] * 2), interpolation=cv2.INTER_NEAREST
        ),
        "bilinear": lambda x: cv2.resize(
            x, (x.shape[0] * 2, x.shape[1] * 2), interpolation=cv2.INTER_LINEAR
        ),
        "bicubic": lambda x: cv2.resize(
            x, (x.shape[0] * 2, x.shape[1] * 2), interpolation=cv2.INTER_CUBIC
        ),
    }
    ...
    for apf_name, apf in approximation_filters.items():
        for inf_name, inf in interpolation_filters.items():
            ...
            for filename in track(
                os.listdir(SOURCE_PATH), description=f"{apf_name}_{inf_name}"
            ):
                image = cv2.imread(
                    os.path.join(SOURCE_PATH, filename), cv2.IMREAD_GRAYSCALE
                )
                pyramid = list(pyramid_builder(image, apf, inf))
                pyramid_images = merge_image(pyramid)
                cv2.imwrite(
                    os.path.join(dist_path, f"{filename.split('.')[0]}_pyramid.jpg"),
                    pyramid_images,
                )
